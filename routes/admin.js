...
router.get('/home', verifyLogin, async (req, res, next) => {
  ...
  adminHelper.getRequests().then((result) => {
    ...
    res.render('admin/index', {
      admin: true, notify: req.session.autoRequest, changedPass: req.session.adminPassChange, total_auto,
      total_user, accepeted_auto, revenue, title: "Home"
    });
    req.session.adminPassChange = null;
  }).catch(() => {
    res.render('admin/index', {
      admin: true, changedPass: req.session.adminPassChange, total_auto,
      total_user, accepeted_auto, revenue, title: "Home"
    });
    req.session.adminPassChange = null;
  });
