...
router.get('/edit-profile', verifyLogin, (req, res, next) => {
  res.render('auto/edit-profile', {
    auto: true, autoBooked: req.session.autoBooked,
    autoDriver: req.session.autoDriver, title: "Edit profile"
  });
});
...
router.get('/home', verifyLogin, async (req, res, next) => {
  ...
  res.render('auto/index', {
    ...
    total_booking, total_travel, total_place, ratings, title: "Auto Home"
  });
...
router.get('/booking', verifyLogin, (req, res, next) => {
  autoHelper.getbooking(req.session.autoDriver._id).then((booking) => {
    ...
    res.render('auto/booking', {
      auto: true, autoDriver: req.session.autoDriver,
      booking, drive: req.session.drive, autoBooked: req.session.autoBooked, title: "Auto bokings"
    });
  }).catch(() => {
    req.session.catchErr = true
    res.render('auto/booking', {
      auto: true, autoDriver: req.session.autoDriver,
      catchErr: req.session.catchErr, drive: req.session.drive, title: "Auto bookings"
    });
  });
...
router.get('/travel-places', verifyLogin, (req, res, next) => {
  autoHelper.getPlaces(req.session.autoDriver._id).then((places) => {
    res.render('auto/places', {
      auto: true, autoDriver: req.session.autoDriver,
      places, autoBooked: req.session.autoBooked, title: "Travel places"
    });
  });
});
...
router.get('/add-destinations', verifyLogin, (req, res, next) => {
  autoHelper.getKm().then((km) => {
    res.render('auto/add-destinations', {
      auto: true, autoDriver: req.session.autoDriver,
      km, autoBooked: req.session.autoBooked, title: "Add places"
    });
  });
});
...
router.get('/users', verifyLogin, (req, res, next) => {
  autoHelper.getUsersTravelled(req.session.autoDriver._id).then((drives) => {
    res.render('auto/users', {
      auto: true, autoDriver: req.session.autoDriver,
      autoBooked: req.session.autoBooked, drives, title: "Users travelled"
    });
  }).catch(() => {
    res.render('auto/users', {
      auto: true, autoDriver: req.session.autoDriver,
...
router.get('/edit-place/:id', verifyLogin, (req, res) => {
  autoHelper.getPlaceDetails(req.params.id).then((charges) => {
    autoHelper.getKm().then((km) => {
      res.render('auto/edit-places', {
        auto: true, autoDriver: req.session.autoDriver,
        autoBooked: req.session.autoBooked, charges, km, title: "Edit place"
      });
    });
  });
...
router.get('/feedback', verifyLogin, (req, res) => {
  autoHelper.getFeedbacks(req.session.autoDriver._id).then((feedbacks) => {
    res.render('auto/feedback', {
      auto: true, autoDriver: req.session.autoDriver, feedbacks,
      autoBooked: req.session.autoBooked, title: "Feedbacks"
    });
  });
});
