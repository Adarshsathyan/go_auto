...
module.exports = {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (autoDriver) {
                ...
                db.get().collection(collections.AUTO_COLLECTION).insertOne(autoDetails).then((response) => {
                    respons.autoDriver = response.ops[0];
                    respons.status = true;
                    resolve(respons);
                });
            };
    ...
    changePaidStatus: (autoId) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collections.AUTO_COLLECTION).updateOne({ _id: objectId(autoId) }, {
                $set: {
                    status: "1"
                }
            }).then(() => {
    ...
    completeDrive: (autoId) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collections.BOOKING_COLLECTION).updateOne({ autoId: objectId(autoId) }, {
                $set: {
                    status: "1"
                }
            }).then(() => {
    ...
    updatePlace: (placeId, details) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collections.TRAVELPLACES_COLLECTION).updateOne({ _id: objectId(placeId) }, {
                $set: {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (auto) {
                bcrypt.compare(details.password1, auto.password).then((status) => {
                    if (status) {
                        db.get().collection(collections.AUTO_COLLECTION).updateOne({ _id: objectId(details.autoId) }, {
                            $set: {
                                password: password
                            }
                        }).then(() => {
    ...
    changeForgotPass: (details) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (auto) {
                details.password = await bcrypt.hash(details.password, 10);
                db.get().collection(collections.AUTO_COLLECTION).updateOne({ _id: objectId(details.userId) }, {
                    $set: {
