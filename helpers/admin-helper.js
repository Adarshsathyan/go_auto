...
module.exports = {
    ...
    updateCharge: (chargeId, details) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collections.TRAVELCHARGE_COLLECTION).updateOne({ _id: objectId(chargeId) }, {
                $set: {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (admin) {
                bcrypt.compare(details.password1, admin.password).then((status) => {
                    if (status) {
                        db.get().collection(collections.ADMIN_COLLECTION).updateOne({ _id: objectId(details.adminId) }, {
                            $set: {
                                password: password
                            }
                        }).then(() => {
